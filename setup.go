...
func setupCodexMCP(binaryPath string) error {
	...
	upstreamModel := gcfg.ProxyModel
	if upstreamModel == "" {
		upstreamModel = "gpt-5.1"
	}

	profile := fmt.Sprintf(`# Codex proxy profile — generated by aipmc setup codex
# Usage: codex -p proxy
#        codex -p proxy exec "hello"
model = "%s"
...
env_key_instructions = "not needed for local proxy — key is set on the proxy side"
`, modelName, proxyPort)

	if err := os.WriteFile(profilePath, []byte(profile), 0644); err != nil {
		return fmt.Errorf("cannot write proxy profile: %w", err)
	}
