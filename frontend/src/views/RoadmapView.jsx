...
import { Button, Card, Col, Collapse, Empty, Form, Input, List, Progress, Row, Select, Space, Tag, Typography } from "antd";
...
function docPathFromFile(filePath) {
  ...
  if (!path) return "";
  if (path.startsWith("doc/") || path.startsWith("docs/")) return path;
  return /\.(md|mdx|rst|txt)$/i.test(path) ? path : "";
}

function RelatedContext({ context, onOpenDoc, onOpenCommit, onDeleteNote, taskNotes = [] }) {
  const allLogs = useMemo(() => {
    const combined = [
      ...
      ...taskNotes.map(n => ({
        type: n.mode === "review" ? "review" : "note",
        date: n.created_at,
        content: n.content,
        id: n.id
      }))
  ...
  if (!context?.idea && !allLogs.length && !context?.docs?.length) return null;
        ...
        <div className="tree-artifact-row">
          ...
          <div className="tag-wrap">
            {context.docs.slice(0, 4).map((doc) => (
              <Tag key={doc.path || doc} className="tree-doc-tag" color="processing"
                  ...
                  <div key={item.id} className="timeline-entry">
                    ...
                    <div className="timeline-content">
                      ...
                      <div className="timeline-body"
                        style={item.type === "commit" ? { cursor: "pointer" } : {}}
                        onDoubleClick={item.type === "commit" ? () => onOpenCommit?.(item.id, item.content) : undefined}>
                        ...
                        {item.type === "review" && (
                          <Text type="secondary" style={{ marginLeft: 8, cursor: "pointer", fontSize: 11 }}
                            onClick={() => onDeleteNote?.(item.id)}>删除</Text>
                        )}
                      </div>
                    </div>
                  </div>
...
function TaskCard({ task, context, onOpenDoc, onOpenCommit, onDeleteNote, onUpdateTask, onSubmitReview, taskNotes, planTitle, roadmapTitle }) {
  const [reviewNote, setReviewNote] = useState("");

  const staleDays = useMemo(() => {
    const lastActive = new Date(task.updated_at);
    return Math.ceil((new Date() - lastActive) / (1000 * 60 * 60 * 24));
  }, [task.updated_at]);
  ...
  const statusLabel = useMemo(() => {
    if (task.status === "done") return `已完成 ${durationDays}天`;
    if (task.status === "blocked") return "阻塞";
    if (task.status === "todo") return "待开始";
    if (staleDays > 2) return `${staleDays}天未更新`;
    return `进行中 ${durationDays}天`;
  }, [task.status, durationDays, staleDays]);

  function handleSubmitReview() {
    if (!reviewNote.trim()) return;
    onSubmitReview?.(task.id, reviewNote.trim());
    setReviewNote("");
  }
      ...
      <div className={`tree-node-row tree-node-row--task ${task.status === "in_progress" ? "status-pulse" : ""}`}>
        <Space direction="vertical" size={2} style={{ flex: 1 }}>
          ...
          <Space wrap>
            ...
            <Tag color={task.status === "done" ? "green" : staleDays > 2 ? "warning" : "default"} style={{ fontSize: 10 }}>
              {statusLabel}
            </Tag>
          </Space>
        ...
        <Space wrap className="tree-node-meta">
          ...
          {!!context?.docs?.length && <Tag>{context.docs.length} docs</Tag>}
          {!!context?.commits?.length && <Tag color="cyan">{context.commits.length} commits</Tag>}
      ...
      <Space direction="vertical" size={8} style={{ width: "100%" }}>
        <Space wrap>
          ...
          <Select size="small" value={task.status} style={{ width: 110 }}
            onChange={(val) => onUpdateTask?.(task.id, val)}
            options={[
          ...
          />
          <Button size="small" type="link" onClick={() => onUpdateTask?.(task.id, "done")}>Approve</Button>
          <Button size="small" type="link" onClick={() => onUpdateTask?.(task.id, "todo")}>Reopen</Button>
        </Space>
        ...
        <RelatedContext context={context} onOpenDoc={onOpenDoc} onOpenCommit={onOpenCommit} onDeleteNote={onDeleteNote} taskNotes={taskNotes || []} />
        <Input size="small" placeholder="输入审核意见，回车提交..."
          value={reviewNote}
...
function taskItems(tasks, artifactsByTask, onOpenDoc, onOpenCommit, onUpdateTask, onSubmitReview, onDeleteNote, taskNotesMap, planTitle, roadmapTitle) {
...
function PlanDetails({ plan, tasks, artifactsByTask, busy, onAdvancePlan, onOpenDoc, onOpenCommit, onUpdateTask, onSubmitReview, onDeleteNote, taskNotesMap, docs, roadmapTitle }) {
  return (
    <Space direction="vertical" size={12} style={{ width: "100%" }}>
      ...
      {(() => {
        ...
        tasks.forEach(t => (t.related_docs || []).forEach(d => planDocs.add(d)));
        if (!planDocs.size) return null;
        return (
          <div className="tree-artifact-row">
            <Text type="secondary">Design Docs</Text>
            <div className="tag-wrap">
              {[...planDocs].map(docPath => (
                <Tag key={docPath} className="tree-doc-tag" color="processing"
                  onClick={() => onOpenDoc?.(docPath)}>
                  {docPath}
                </Tag>
              ))}
            </div>
      ...
      {tasks.length ? (
        <Collapse className="task-tree-collapse" defaultActiveKey={[]}
          items={taskItems(tasks, artifactsByTask, onOpenDoc, onOpenCommit, onUpdateTask, onSubmitReview, onDeleteNote, taskNotesMap, plan.title, roadmapTitle)}
        />
      ) : (
...
function RoadmapTree({ roadmap, artifactsByTask, busy, onAdvancePlan, onOpenDoc, onOpenCommit, onUpdateTask, onSubmitReview, onDeleteNote, taskNotesMap, docs }) {
  return (
    <Collapse className="roadmap-tree-collapse" defaultActiveKey={[roadmap.id]}
      items={[{
        key: roadmap.id,
        label: (
        ...
        children: (
          <Space direction="vertical" size={12} style={{ width: "100%" }}>
            {roadmap.plans.length ? (
              <Collapse className="plan-stack-collapse" defaultActiveKey={[]}
                items={roadmap.plans.map((plan) => ({
                  ...
                  children: <PlanDetails plan={plan} tasks={roadmap.tasksByPlan[plan.id] || []} artifactsByTask={artifactsByTask}
                    busy={busy} onAdvancePlan={onAdvancePlan} onOpenDoc={onOpenDoc} onOpenCommit={onOpenCommit}
                    onUpdateTask={onUpdateTask} onSubmitReview={onSubmitReview} onDeleteNote={onDeleteNote}
                    taskNotesMap={taskNotesMap} docs={docs} roadmapTitle={roadmap.title} />,
                }))}
              />
            ) : (
            ...
            {!!roadmap.unplannedTasks.length && (
              <Collapse className="task-tree-collapse" items={[{
                key: "unplanned",
                label: (
                ...
                children: <Collapse className="task-tree-collapse"
                  items={taskItems(roadmap.unplannedTasks, artifactsByTask, onOpenDoc, onOpenCommit, onUpdateTask, onSubmitReview, onDeleteNote, taskNotesMap, roadmap.title, "")} />,
              }]} />
            )}
          </Space>
        ),
...
export default function RoadmapView({ roadmaps, plans, tasks, taskNotes = [], visions, commits = [], docs = [], ideas = [], busy, onCreateRoadmap, onAdvancePlan, onUpdateTask, onSubmitReview, onDeleteNote, onOpenCommit }) {
  ...
  const artifactsByTask = useMemo(() => {
    ...
    return new Map((tasks || []).map((task) => {
      ...
      const taskDocs = [...docPaths].map((path) => docsByPath.get(path)).filter(Boolean);
  ...
  return (
    <div className="view-stack">
          ...
          <Row gutter={16}>
            ...
            <Col xs={24} md={5}>
              <Form.Item label="Vision">
                <Select allowClear value={roadmapForm.vision_id || undefined}
            ...
            <Col xs={12} md={2}>
              <Form.Item label="Priority">
                <Select value={roadmapForm.priority} onChange={(value) => setRoadmapForm({ ...roadmapForm, priority: value })}
                  options={[{ value: "P0" }, { value: "P1" }, { value: "P2" }]} />
              </Form.Item>
            </Col>
            <Col xs={12} md={3}>
              <Form.Item label="Status">
                <Select value={roadmapForm.status} onChange={(value) => setRoadmapForm({ ...roadmapForm, status: value })}
                  options={[{ value: "planned" }, { value: "active" }, { value: "done" }]} />
              </Form.Item>
            </Col>
          </Row>
      ...
      <List grid={{ gutter: 16, column: 1 }} dataSource={roadmapTree}
        renderItem={(roadmap) => (
          <Card className="console-card roadmap-card" key={roadmap.id}
            title={
              <Space wrap>
                <Text strong>{roadmap.title}</Text>
            ...
            <div className="roadmap-tree-section">
              <Space direction="vertical" size={12} style={{ width: "100%" }}>
                <Space direction="vertical" size={2}>
                  ...
                  <Text type="secondary">Details are collapsed by default. Click to expand, double-click commit entries to jump.</Text>
                </Space>
                <RoadmapTree roadmap={roadmap} artifactsByTask={artifactsByTask} busy={busy}
                  onAdvancePlan={onAdvancePlan} onOpenDoc={setReadingDoc} onOpenCommit={onOpenCommit}
                  onUpdateTask={onUpdateTask} onSubmitReview={onSubmitReview} onDeleteNote={onDeleteNote}
                  taskNotesMap={taskNotesMap} docs={docs} />
              </Space>
            </div>
          </Card>
        )}
