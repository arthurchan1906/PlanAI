...
// UnnestGoal is the exported wrapper for unnestGoal, used by the injection
// cache-read path so stale/nested goals from L2 summaries are normalized
// even when the summary was generated by an older binary.
func UnnestGoal(goal string) string {
	return unnestGoal(goal)
}

// NormalizeSummaryGoal parses an L2 summary JSON and replaces any nested
// "goal" value with its unnested form, so the stored summary column never
// carries string-escaped or markdown-wrapped goal JSON. Returns the input
// unchanged when it is not parseable JSON.
func NormalizeSummaryGoal(summary string) string {
	if summary == "" {
		return summary
	}
...
func unnestGoal(goal string) string {
	...
	if strings.HasPrefix(cleaned, "{") {
		...
		// Step 2b: loose extraction — the value is string-escaped JSON,
		// contains invalid UTF-8, or is truncated. Fall back to a regex
		// scan for the first "goal" value instead of giving up.
		if ig := looseExtractGoal(cleaned); ig != "" {
			return ig
		}
	}
...
// looseExtractGoal finds the value of the first "goal" key in a malformed
// JSON-ish string (string-escaped JSON, invalid UTF-8, or truncated output).
func looseExtractGoal(s string) string {
	re := regexp.MustCompile(`"goal"\s*:\s*"`)
	searchFrom := 0
	var valueFrom int
	for {
		m := re.FindStringIndex(s[searchFrom:])
		if m == nil {
			return ""
		}
		abs := searchFrom + m[0]
		// Only accept a match that looks like a JSON key (preceded by '{' or
		// ',' at object level), so prose mentioning `"goal": "` inside a
		// session transcript is not misread as the goal value.
		pre := abs - 1
		for pre >= 0 && (s[pre] == ' ' || s[pre] == '\t' || s[pre] == '\n' || s[pre] == '\r') {
			pre--
		}
		if pre >= 0 && (s[pre] == '{' || s[pre] == ',') {
			valueFrom = abs + m[1] - m[0]
			break
		}
		searchFrom = abs + 1
	}
	...
	if out != "" {
		if unq, err := strconv.Unquote(`"` + out + `"`); err == nil {
			out = unq
		}
	}
	return out
