...
RUNTIME_DIRNAME = ".planai"
DB_FILENAME = "planai.json"
CONFIG_FILENAME = "config.json"
DEFAULT_WEB_HOST = "127.0.0.1"
DEFAULT_WEB_PORT = 8011
RECOVERY_DIR = Path.home() / ".codex" / "memories"
RECOVERED_DB_PATH = RECOVERY_DIR / "planai.recovered.db"
...
def find_runtime_dir(start: Optional[Path] = None) -> Optional[Path]:
    override = os.getenv("PLANAI_HOME") or os.getenv("PROJECT_OS_HOME")
    if override:
...
def get_connection() -> sqlite3.Connection:
    ...
    if not db_path.exists():
        raise FileNotFoundError(f"PlanAI database not found: {db_path}")
...
def fetch_canon() -> Dict[str, Any]:
    ...
    try:
        ...
        if not row:
            raise FileNotFoundError("Canon not found in planai.db")
        items = conn.execute(
