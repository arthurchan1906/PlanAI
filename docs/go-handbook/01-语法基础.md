...
**AIPMC 为什么用 Go**：编译后是一个单文件（`aipmc` / `aipmc.exe`），不需要 Python 环境、不需要 `pip install`、不需要 Node.js。用户复制文件就能跑。
...
Go 的 `embed` 指令在**编译时**把文件嵌进二进制。AIPMC 的整个 Web UI（React + Ant Design）就靠这一行打包进 `aipmc`。
