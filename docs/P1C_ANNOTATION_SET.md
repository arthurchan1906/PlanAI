...
已知健康时段混入，验证 L2 不误判：
- c0ad2534 06-23 14h：正常（讨论+检索，build=0 自发=1）
- c0ad2534 06-24 11h：修复验证期（纠偏→根因→修复，非盲试）

执行方式：将健康时段构造为 deadloop_confirm 输入跑 L2，人工核对 L2 是否判 is_deadloop=false（不误判）。代码侧需补「健康时段注入」工具后执行。
