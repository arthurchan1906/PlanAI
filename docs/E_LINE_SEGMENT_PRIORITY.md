...
| [项目编码规范] guidelines | 4（降） | **保底 200 + 剩余** | ✅ **保底 200B**（Claude 8/31 补充：保底工具直映射关键句） |
| 最近的 session goals | 5（降） | 剩余 | ❌ 有空间才写 |

**策略调整**：guidelines 从「600 独立（恒定）」降为「**可让位但保底 200B**」——通用规范价值低于实时任务上下文（Claude 已同意），但**必须保底 200B** 让 P0 ② 的工具直映射关键句（如「查决策→list_decisions/get_decision」）能进（Claude 8/31 补充，避免 P0 ② 成果被挤掉）。

**P0 ①②③ 互不挤**：P0 ① 钩子（record_bug 提醒）→ warn/act 保底；P0 ② 指南（工具直映射）→ guidelines 保底 200；P0 ③ 到达率治理→ 高优段先写、低优让位。
...
→ warnings → ⚠️待处理(actionItems) → [项目编码规范](guidelines, **保底 200**)
...
- **低优段**（guidelines/goals/vision）最后写、被裁时先裁——guidelines 从「独立 600B 先占」改为「在 warn/act 之后用剩余，但**保底 200B**（裁剪到最后留 200B 给工具直映射关键句）」。
...
| guidelines | 4 | 第6段 | **保底 200 + 剩余** | 目标（降级但保底） |
| goals | 5 | 第7段 | 剩余 | 目标 |
...
- **guidelines 保底 200B**：工具直映射关键句（P0 ② 指南）在裁剪后仍可达（Claude 8/31 补充）
