...
- 量化指标：Agent 主动调用修复类工具（record_commit 修复 orphan、update_task_status、link_entities）次数；对话中出现注入内容响应（"已通过 AIPMC 梳理完当前项目状态"等）
- 数据源：discussion_log
...
> 4. **用户侧接力打标**——用户转达当下标记「这条转达依赖 AIPMC 上下文吗」，即时反事实测量（回忆偏置最小，成本为零——用户本来就在转达）；形态：极简 CLI/MCP 打标工具，**≤1 按键量级**（schema 最小化：`relay_pair, depends_on_aipm, ts`，不做任何额外输入，否则用户放弃——codex 8/13 约束）
...
- **口径边界（8/13 审核注记）**：项目根过滤仅在路径含 `/.pmai/data/pmai.db` 时生效；`PMAI_HOME`/env 模式无项目根，回退全量口径（安全不崩溃，覆盖率会偏低）
...
- 设计意图：v1.14 共识主风险是「隔小时接力断链」而非并发碰撞；断链事件是 AIPMC 声称要防的事故
...
- 设计意图：锚点和行为链只能测 Agent 侧，但 AIPMC 的核心消费方是转达中枢（人）——人的判断对 metrics 不可见
- 量化指标：转达消息中「依赖 AIPMC 上下文」打标比例
- 数据源：打标工具（待建，占位——极简 CLI/MCP，转达当下标 y/n，≤1 按键）
- 基线：待工具上线
- 目标：**阶段条件（codex 8/13 修正——方向依赖系统阶段，不写阶段会得出「工具越有用越没用」）**：
  - L1/编排台落地**前**：AIPMC 是唯一转达管道，打标「依赖 AIPMC」比例**高 = 工具在承重（正向证据）**
...
7. **F 层纪律三（8/13）**：无效结论的门槛——当送达率/样本量不足以产生可观察差异时，行为观测判定为「不可测」而非「无效」（观测不到差异 ≠ 无差异）。等条件改善（送达率/样本量达到统计功效）后再测。先例：fileAssoc 1/24 送达下测 agent 行为差异（8/13 F2 观测点 2 后）。
8. **F 层执行路径（8/13 收敛）**：先修仪器（EPIC「可见性漏斗」W1 suppressed ids/priority/source_segment + W2 D2 三口径 + W3 metrics `--since`）→ 固化锚点（F1-F6）→ 行为证据链 → 自然对照（same_content 臂、跨项目差异部署）。对照实验最后，不故意降级。
