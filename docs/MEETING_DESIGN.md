...
> **[DEPRECATED — v1 默认路径]** 本文描述「实时同步会议 + turn 状态机 + wait + 自动仲裁」原型，基于 2026-06-06 讨论。  
> **2026-06-15** 三方 Live Case Study 结论：第三方 Code Agent 的 v1 默认协作见 **[`COLLABORATION_DESIGN.md`](COLLABORATION_DESIGN.md)**。  
> 下文保留作历史参考；§3–§7 中的 wait / 自动仲裁 **不在 v1 实现**。

> 多 Agent 会议协作的早期完整设计。基于 2026-06-06 讨论总结。
