...
- **8/26 口径变更（跨项目污染修复）**：日志全局共享（`~/.aipmc/logs/aipmc.log`
  含所有项目注入行），此前 M1a 分母混入其他项目行致对账虚低（8/19-24 实测
  EncryptDrive 注入行 283/555/823 混入、aipmc 对账 12-14%——8/18 87.4%/8/25
  98.6% 纯净窗口正常，根因是对账口径非捕获层失效）。**8/26 09:41 重启后** proxy
  日志带 `project=` 绝对路径（单点推导 pmdb.FindPath+ProjectRoot，正常项目与
  env PMAI_HOME 模式统一；8/26 实测 Dir² 少一层停在 .pmai 的错值已修并有测试锁定），
  M1a 分母按当前项目过滤；首个纯净窗口（8/26 09:41 起）对账 100%（样本小待积累）；
  历史无 `project=` 行不过滤，**全窗口对账仅对日志存活期内有效**（20MB keep 7
  归档只扫当前文件，历史对账不可复现）——对账解读须带窗口注记
