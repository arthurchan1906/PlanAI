...
|---|---|---|---|---|---|
| [01] c0ad2534 15:00-15:30 | 死循环该用未用 | nm/objdump/grep DZ_Pal + 头文件对比，零 aipm 调用 | uncertain（零 aipm 事实成立，「该用」待 L2） | 待填 | 待填 |
| [08] 01a013f3 8/19 15:33→8/20 08:49 | 重复验证点 | fix commit 后 9 次「Xcode Run 再测」请求 | true_positive | 待填 | 待填 |
| [10] 01a013f3 8/18 17:54→8/19 09:07 | 自建记录利用 | create_task 后收尾，次日 09:07 首次检索（扣休眠 22min） | uncertain（收尾创建方向性弱） | 待填 | 待填 |

三方判定一致率 = 三方同判数 ÷ 抽检时段数（填齐后计算）。
...
2. **legacy 格式漏计**（[04] 实证）：c0ad2534 类 6 月 session 的 aipm 工具行解析不完整，`hint_missed` 与检索计数类检测点存在解析漏报——P1 需在 parse.go 或检测层补 `📡 aipm_` text 行识别。
