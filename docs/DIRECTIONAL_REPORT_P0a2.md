...
**主动触发**
- `deadloop_no_aipm` ×2：15:00-16:00（build=16 自发=0）、16:00-17:00（build=11 自发=0）——死循环时段该用未用实证命中（对照物：全部 38 条 aipm 调用均发生在 17:20 用户提示后）。
- `hint_responded` ×8：17:20（aipm=13）、17:38（8）、09:11（7）、10:09（4）、10:44（13）、10:45（7）、11:43（1）——用户提示后窗口内主动检索 ✓。
- `hint_missed` ×3：14:02「查看opencode的分析」（窗口内 0 aipm——同日 14:02/15:50 两次提示均未用 discussion 工具，直至 17:20 才首次）、10:48「aipm中又记录 但是可能你搜索错了位置」（提示后 30 分钟未重搜）、11:46「应该在aipm合适的其他地方记录」（窗口内无 aipm，记录动作 11:44 已发生——L1 时间窗局限，见 §3）。
...
**重复验证点**：3 候选。关键 episode（8/19 15:33 commit → 8/20 10:40 commit，无 fix commit 间隔）请求 12 次，含锚点 17:25 + 09:05 —— 即用户 10:16 抗议「为什么你要一而再再而三的测试」所指。

**自建记录利用**：8 候选，每条创建事件后首次 aipm 检索前均有 ≥5 条工作记录。锚点候选 = 8/19 15:32 `record_bug` → 17:29 才首次检索（工作 41 条，延迟 117 分钟）。其余 7 条同类（create_task/record_bug/record_decision 创建后延迟 18-912 分钟才检索，含 17:54 create_task 隔夜 912 分钟、17:18 record_bug 至会话结束未检索）。

---

## 3. 已知局限（方向性报告如实记录）

1. **L1 时间窗局限**：`hint_missed` 判定依赖 30 分钟窗口——c0ad2534 11:46「应该在aipm合适的其他地方记录」前 11:44 已有记录动作，属窗口外已响应，L1 误报为 missed；精确响应判定归 L2。
