...
AIPMC 是一个**设计哲学极其成熟**的项目。它知道自己不做什么（不加 LLM、不替 Agent 做决策），知道自己该做什么（高密度上下文注入、结构化反思提示）。在这个定位上，它已经做得很好了。
...
1. **不要加内置 LLM** — DESIGN.md Decision 1 完全正确。AIPMC 的价值在"让 Agent 变聪明"，不是在内部跑 LLM
2. **不要改 Go→Python** — 单文件部署是 AIPMC 的核心竞争力。Go 的并发模型和编译特性在这个场景下比 Python 更合适
