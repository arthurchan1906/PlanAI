...
def bootstrap_project_db() -> bool:
    bootstrap_state(
        {
            "meta": {
                "project_name": "planai-project",
                "project_type": "ai-coding-project-manager",
                "runtime_root": ".planai",
                "runtime_database": ".planai/data/planai.json",
                "runtime_config": ".planai/config.json",
                "ai_read_order": [
                    ".planai/data/planai.json:canon",
                    ".planai/data/planai.json:tasks",
                    ".planai/data/planai.json:doc_records",
                    "README.md",
                    "pyproject.toml",
                ],
            },
            "canon": {
                ...
                "version_scope": ["项目级 .planai 隐藏目录初始化", "CLI 任务/提交/决策/日报管理", "内置 Web 查看与编辑", "pip 可安装分发", "移除旧 planai 路径残留"],
                "avoid_now": ["引入额外后端框架依赖", "依赖团队协作或多用户同步", "把运行时数据放回代码目录"],
                "top_tasks": ["清理旧包名和路径硬编码", "保证 planai init 可在任意项目创建 .planai", "保证 planai-web 使用标准库直接启动"],
            ...
            "decisions": [
                {"id": "decision-runtime-hidden-dir", "title": "运行时数据放入项目隐藏目录", "date": today(), "status": "accepted", "background": "工具会被复制到不同目录中使用，代码路径不应承担项目数据目录职责。", "decision": "在用户项目根目录使用 .planai/ 保存数据库和运行配置，并支持从子目录向上发现该目录。", "impact": ["每个项目独立保存数据和端口配置"], "alternatives": ["继续把数据库放在源码目录"], "related_tasks": ["task-runtime-layout"], "updates_canon": True},
            ...
            "tasks": [
                {"id": "task-runtime-layout", "title": "统一 .planai 运行时目录和初始化逻辑", "status": "in_progress", "priority": "P0", "phase": "foundation", "acceptance": ["planai init 会创建 .planai/data/planai.json 和 .planai/config.json", "CLI 和 Web 从子目录也能定位到同一项目数据库"], "related_docs": ["bootstrap.py", "store.py", "run_server.py"], "related_decisions": ["decision-runtime-hidden-dir"], "last_note": "", "updated_at": today()},
